...
-- 성적 일람표를 보고 싶은데, 학생의 학번과 이름이 포함된 리스트를 보고싶다.
-- 두개 table, tbl_score, tbl_student를 연동하여 리스트를 조회해야한다.
-- 이러한 기법을 JOIN 이라고 한다.
...
--------------------------------------------------------------------------------
-- 완전 JOIN
-- sc_score table에 있는 학번(Num[ )의 정보는 반드시 st_student에 있다 라는 전제하에
-- 결과가 원하는 대로 나온다.
...
-- 학생 테이블에 있는 정보만 연결해 보여주는 방식의 JOIN
-- 데이터 검증용으로도 많이 사용함
DELETE FROM tbl_student WHERE st_num > '20050';
...
-- 키값으로 오른쪽 table에서 값을 찾은후 있으면 PROJECTION에 나열된
-- 칼럼 값을 표시하고 만약 없으면 (null)이라고 표시한다.

-- 왼쪽 테이블의 데이터가 잘 입력되었나 검증하는 용도로 많이 사용되고 
-- 아직 FK 설정이 되지 않은 table간에 정보를 리스트 업하는 용도로 사용된다.

SELECT sc_num, st_name, sc_kor, sc_eng, sc_math, sc_music, sc_art 
FROM tbl_score -- 모든 data
    LEFT JOIN tbl_student -- 검증할 table
        ON sc_num = st_num;
------------------------------------------------------------------------------
