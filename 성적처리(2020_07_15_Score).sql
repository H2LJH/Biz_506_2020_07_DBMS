...
-- DEFAULT : 해당하는 칼럼에 데이터를 지정하지 않고 INSERT를 수행하면 
...
-- AS(Alias) : Table의 칼럼명, 연산수식등에 별명을 부여하는 사용하는 방법
...
-- SELECTION을 적용하여 구현
-- 학번이 20001 - 20010까지 학생의 리스트만 보고싶을때
SELECT * FROM tbl_score WHERE sc_num BETWEEN '20001' AND '20010';
...
-- 학번이 20001-20010인 학생들만 추출하여 과목별 평균을 계산하라  -----------------
SELECT AVG(sc_kor)   AS 국어평균,  
       AVG(sc_eng)   AS 영어평균,  
       AVG(sc_math)  AS 수학평균,  
       AVG(sc_music) AS 음악평균,  
...
-- 학번이 20001-20010인 학생들만 추출하여 과목별 점수 ----------------------------
SELECT sc_num, (sc_kor)   AS 국어,  
               (sc_eng)   AS 영어,  
               (sc_math)  AS 수학,  
               (sc_music) AS 음악,  
               (sc_art)   AS 미술 
...
-- 의미 없는(DUMY) 칼럼을 '==과목총점=='이라는 값으로 만들었다.
-- 통계, 집계함수를 사용할때 통계, 집계함수로 묶이지 않은 칼럼은
...
UNION ALL SELECT '==과목총점==', --sc_num의 칼럼의 개수와 맞추기 위해 작성
       SUM(sc_kor)   AS 국어,  
       SUM(sc_eng)   AS 영어,  
       SUM(sc_math)  AS 수학,  
       SUM(sc_music) AS 음악,  
